...
for simu100 in range(100):
    ...
    for tim in range(CombNum):
        ...
        PvalueTp = wre_test( T, X, P, method=SADDLEPOINT )
    ...
    np.savetxt(outdir+'genesets'+simuIndex+str(simu100)+'.csv', subsetTop, delimiter = ',') 



np.savetxt(outdir+'results'+simuIndex+'.csv', rankingVec, delimiter = ',') 
